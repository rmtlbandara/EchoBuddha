...
check("complete production crawl", crawl.length === 344 && crawl.every((row) => !row.fetch_error), `${crawl.length} rows; ${crawl.filter((row) => row.fetch_error).length} fetch errors`);
check("production inventory states", stateCount("INDEXABLE_CANONICAL_200") === 149 && stateCount("NOINDEX_USER_PAGE_200") === 186 && stateCount("PERMANENT_REDIRECT") === 3 && stateCount("TEMPORARY_REDIRECT") === 1, `149 indexable; 186 noindex; 3 permanent redirects; 1 temporary normalization`);
check("full intended contract", contract.length === 344 && contract.every((row) => row.result === "PASS"), `${contract.filter((row) => row.result === "PASS").length}/${contract.length}`);
...
check("canonical/noindex", canonicals.length === 335 && canonicals.every((row) => row.canonical_result === "PASS" && row.robots_result === "PASS" && row.sitemap_result === "PASS"), `${canonicals.length} applicable rows`);
