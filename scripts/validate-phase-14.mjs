...
check("complete production crawl", crawl.length === 349 && crawl.every((row) => !row.fetch_error), `${crawl.length} rows; ${crawl.filter((row) => row.fetch_error).length} fetch errors`);
check("production inventory states", stateCount("INDEXABLE_CANONICAL_200") === 154 && stateCount("NOINDEX_USER_PAGE_200") === 186 && stateCount("PERMANENT_REDIRECT") === 3 && stateCount("TEMPORARY_REDIRECT") === 1, `154 indexable; 186 noindex; 3 permanent redirects; 1 temporary normalization`);
check("full intended contract", contract.length === 349 && contract.every((row) => row.result === "PASS"), `${contract.filter((row) => row.result === "PASS").length}/${contract.length}`);
...
check("canonical/noindex", canonicals.length === 340 && canonicals.every((row) => row.canonical_result === "PASS" && row.robots_result === "PASS" && row.sitemap_result === "PASS"), `${canonicals.length} applicable rows`);
...
check("deployment identity", manifest.includes("ad9fe897916c76fd6883efc351461b49db28ac3f") && manifest.includes("f8f3f4c9-b156-422f-ad19-569abc767750") && manifest.includes("d3da65308348cb5379c76271614288d14f150450") && manifest.includes("a07b1d86-9af9-4d99-97a5-5b0c02c6bdbc") && manifest.includes("Rollback used: NO"), "immutable initial and current authorized releases recorded");
