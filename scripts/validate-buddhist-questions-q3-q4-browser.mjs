...
const port = process.env.Q34_AUDIT_PORT || "4323";
const baseUrl = process.env.Q34_AUDIT_BASE_URL || `http://127.0.0.1:${port}`;
const chromePath = process.env.CHROME_EXECUTABLE_PATH || "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const output = path.join(root, "docs/audits/buddhist-questions-q3-q4-2026-09-14/BROWSER_VALIDATION.json");
...
const expectedNavigation = {
  [routes[0]]: [routes[4], routes[1]],
  [routes[1]]: [routes[0]],
  [routes[3]]: [routes[4]],
  [routes[4]]: [routes[3], routes[0]]
};
...
try {
  if (!process.env.Q34_AUDIT_BASE_URL) {
    ...
    for (const route of routes) {
      for (const viewport of viewports) {
        ...
        const pass = response?.status() === 200
          ...
          && (route !== routes[2] || pageFacts.cardCount === 4)
          && (route === routes[2] || JSON.stringify(pageFacts.navigationLinks) === JSON.stringify(expectedLinks));
        results.push({
          route,
          viewport,
  ...
  if (!report.pass) throw new Error(`Q3/Q4 browser validation failed; inspect ${path.relative(root, output)}`);
  console.log(`Q3/Q4 browser validation passed: ${results.length} route/viewport combinations, zero critical or serious accessibility violations.`);
} finally {
  await stopPreview();
}
