...
const actionRefs = [
  ...
  ["Evidence jobs", "actions/upload-artifact", "043fb46d1a93c77aae656e7c1c64a875d1fc6a0a", "GitHub", "Official", "No", "Yes", "v7", "Yes", "Artifact upload", "Retain digest evidence and bounded retention", "GitHub API ref resolved"],
  ["Deploy", "actions/download-artifact", "3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c", "GitHub", "Official", "No", "Yes", "v8", "Yes", "Artifact boundary", "Verify per-file release manifest after download", "GitHub API ref resolved"]
];
...
csv("phase-9-external-github-cloudflare-settings.csv", extHeaders, [
  ...
  ["Cloudflare", "Production Git build trigger", "Disabled after governed GitHub production environment is ready", "Active: Phase 9 main merge automatically built and deployed", "Yes", "No", "A main merge can deploy before all post-merge checks finish and bypass exact-artifact deployment controls", "Disable the automatic production trigger; then dispatch the governed workflow for a controlled release and verify no competing deployment occurs", "Cloudflare Workers build check and deployment 614698d1-0bc1-4146-ae7b-5467216cb2a2"],
...
csv("phase-9-owner-action-items.csv", ownerHeaders, [
  ...
  ["O-10", "Process", "Freeze unrelated production/index/AdSense changes during AdSense review and do not reapply in Phase 9", "Avoid confounding measurement and policy review", "Documented; no production mutation during implementation", "Yes", "High", "Owner release review"],
  ["O-11", "Cloudflare", "After configuring the GitHub production environment, disable the legacy automatic production deployment on main and make deploy-production.yml the sole normal production path", "The Phase 9 main merge automatically deployed before all post-merge checks finished", "Competing path observed and documented", "Yes", "Critical", "Merge a non-production test change, verify no Cloudflare production deployment occurs, then dispatch and verify one governed exact-SHA release"]
...
const sections = [
  ...
  ["External Owner Actions Required", "Eleven exact GitHub, Cloudflare and process actions are listed in phase-9-owner-action-items.csv. The observed legacy Cloudflare automatic production trigger is explicitly included; no pending setting is represented as complete before dashboard verification."],
