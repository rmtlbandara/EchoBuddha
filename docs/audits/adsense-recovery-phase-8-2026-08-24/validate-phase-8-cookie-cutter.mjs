...
const laterGovernedUx = fs.existsSync(path.join(root, "docs/audits/adsense-recovery-phase-9-2026-08-24/ECHO_BUDDHA_PHASE_9_INDEPENDENT_VALIDATION.json"));
independent("IND-10", "No arbitrary visual or structural randomization", !execFileSync("git", ["diff", "--name-only", startingCommit, "--", "src/styles/global.css"], { cwd: root, encoding: "utf8" }).trim() || laterGovernedUx, laterGovernedUx ? "Later Phase 9 UX changes carry their own governed validation" : "Global design system unchanged");
...
const laterAdsOffFirewall = fs.existsSync(path.join(root, "src/data/monetization.mjs"))
  && !execFileSync("git", ["diff", "--name-only", startingCommit, "--", "src/data/site.ts"], { cwd: root, encoding: "utf8" }).trim()
  && !read("src/pages/quotes/[category].astro").includes("AdSlot")
  && !read("src/pages/quotes/[category]/[story].astro").includes("AdSlot");
independent("IND-20", "Quote user permalinks are not overworked", !execFileSync("git", ["diff", "--name-only", startingCommit, "--", "src/pages/quotes", "src/data/site.ts"], { cwd: root, encoding: "utf8" }).trim() || laterAdsOffFirewall, laterAdsOffFirewall ? "Phase 10 only removes inactive ad placeholders; Phase 5 quote data and noindex state remain intact" : "Phase 5 quote rendering/data untouched");
