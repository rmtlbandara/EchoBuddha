...
const postDeployParity=await readCsv(path.join(out,'phase-0-4-post-deploy-production-parity.csv'));
const postDeploySummary=JSON.parse(await readFile(path.join(out,'phase-0-4-post-deploy-production-summary.json'),'utf8'));
...
check('Post-deploy production parity',postDeployParity.length===336&&postDeployParity.every((row)=>row['Full parity']==='Yes'),`${postDeployParity.filter((row)=>row['Full parity']==='Yes').length}/336`);
check('Post-deploy production index state',postDeploySummary.productionIndexable===193&&postDeploySummary.productionNoindex===143,`${postDeploySummary.productionIndexable} indexable; ${postDeploySummary.productionNoindex} noindex`);
check('Post-deploy sitemap parity',postDeploySummary.productionSitemapUrls===193&&postDeploySummary.sitemapExactMatch===true,`${postDeploySummary.productionSitemapUrls}; exact=${postDeploySummary.sitemapExactMatch}`);
